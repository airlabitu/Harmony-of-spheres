...
void setup() {
  ...
  envelopes = new Envelope[spheres.length];
  ...
  for (int i = 0; i < envelopes.length; i++){
    envelopes[i] = new Envelope(this, spheres[i].track);
    
    spheres[i].track.loop();
    spheres[i].track.amp(spheres[i].vol.getMin());
    spheres[i].vol.setVal(spheres[i].vol.getMin(), millisToFadeOutside);
  }
...
void soundManipulation(Sphere s, int dist) {
  // turn off
  if (dist < s.radius) {
    s.vol.setVal(s.vol.getMax(), millisToFadeInside);   // shift over 100 millis 
    float position = map(dist, 0, s.radius, 0, s.track.duration());
    
    envelopes[s.id-1].setDuration((int)map(dist, 0, s.radius, 100, 1500));
    
    //envelopes[s.id-1].update(position);
    envelopes[s.id-1].update();
    
    //envelopes[s.id-1].update();
  }
  else s.vol.setVal(0, millisToFadeOutside); // shift to min over 100 milllis
  //float position = map(s.vol.getVal(), s.vol.getMin(), s.vol.getMax(), 0, s.track.duration());
  //envelopes[s.id-1].update(position);
  //envelopes[s.id-1].update();
  
  //envelopes[s.id-1].setDuration((int)map(s.vol.getVal(), s.vol.getMin(), s.vol.getMax(), 10, 1500));
