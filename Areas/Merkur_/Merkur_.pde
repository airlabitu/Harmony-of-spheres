...
void setup() {
  ...
  grainTimers = new long [spheres.length];
  grainIntervals = new int [spheres.length];
  
  for (int i = 0; i < grainIntervals.length; i++){
    grainIntervals[i] = 1000;
  }
}
...
void soundManipulation(Sphere s, int dist) {
  ...
  if (millis() > grainTimers[s.getId()-1] + grainIntervals[s.getId()-1]){
    grainTimers[s.getId()-1] = millis();
    s.track.jump(random(0.5, s.track.duration()));
    grainIntervals[s.getId()-1] = (int) random(500, 500*(1+s.vol.getVal()));
    
    //s.vol.setVal(,)
    //if (s.getId() == 1) println("Test", s.getId(), grainTimers[s.getId()-1], millis());
  }
