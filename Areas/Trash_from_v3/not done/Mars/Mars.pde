...
void setup() {
  ...
  spheres[2].enableRate(); // ### rate
  spheres[2].rate.setMinMax(0.92, 1.0);
  ...
  spheres[4].enableDelay(this, 0.3); // 0.3 is the delay tape // ###delay
  spheres[4].delayVal.setVal(0.5, 1); // ### delay value must be between 0.3 and 0.8, or min max must be changed
  spheres[4].enableRate(); // ### rate
  spheres[4].rate.reverse(true);
  //spheres[4].rate.setMinMax(0.92, 1.0);
