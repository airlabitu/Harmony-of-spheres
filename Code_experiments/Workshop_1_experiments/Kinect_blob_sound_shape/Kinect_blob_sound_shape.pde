...
// ToDo
// Make blind spots + interaction points
// Make correct distance calculations for each blob to each interaction point
// Make soundfile structure for all interaction points (array of soundfiles)
...
void setup() {
  ...
  soundfile1 = new SoundFile(this, "hannah_1.wav");
  soundfile1.loop();
...
void draw() {
  ...
  for (Blob b : blobs) {
    ...
    fill(255,255,0);
    ellipse(b.minx+w/2+width/2, b.miny+h/2+height/2, 10, 10);
    
    float d = dist(b.minx+w/2, b.miny+h/2, 293, 259);
    println("Dist: ", b.id, d);
    /*
    float amplitude = map(constrain(d, 0, 200), 0, 200, -1.0, 1.0);
    println(amplitude, b.id);
    soundfile1.amp(amplitude);
    */
    
    float playbackSpeed = map(constrain(d, 0, 200), 0, 200, 0.25, 4.0);
    soundfile1.rate(playbackSpeed);
  } 
...
void makeTriangle(){
  ...
  for (float i = 255; i > 100; i--){
    ...
    t.setScale(i);
    t.setRotation(75);
    t.setCenter(width/4*3,height/4*3);
    //stroke(i, 0, 0);
    fill(255-i, 0, 0);
    t.display();
  }
